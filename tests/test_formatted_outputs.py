...
class TestFormattedOutputs(unittest.TestCase):
    ...
    def test_agent_generate_with_response_format(self):
        ...
        try:
            ...
            try:
                ...
                # If it's not JSON, we still want to verify the content exists
                pass

        except Exception as e:
            # We expect an API call error in most cases due to environment setup,
            # but what matters is that response_format parameter is handled correctly
            self.assertIn(
                "response_format",
                str(e).lower() or "404" in str(e) or "not found" in str(e),
            )
    ...
    def test_response_format_parameter_propagation(self):
        ...
        # The main thing we're testing here is that our code handles
        # the response_format parameter without errors, which means
        # it gets passed to the OpenAI API correctly.
        try:
            ...
            # We expect API errors due to environment setup, but not parameter parsing issues
            pass
